...
definition(
    name        : "Echo Speaks - Lite",
    namespace   : "aloneflower52999",
        ...
        section() {
            if(state?.actionDuplicated) {
                ...
                if(act) {
                    ...
                    addChildApp("aloneflower52999", actChildName(), "${actData?.label} (Dup)", [settings: actData?.settings])
        ...
        section() {
            if(state?.zoneDuplicated) {
                ...
                if(zn) {
                    ...
                    addChildApp("aloneflower52999", zoneChildName(), "${znData?.label} (Dup)", [settings: znData?.settings])
...
def zonesPage() {
    return dynamicPage(name: "zonesPage", nextPage: "mainPage", uninstall: false, install: false) {
        ...
        section() {
            app(name: "zoneApp", appName: zoneChildName(), namespace: "aloneflower52999", multiple: true, title: inTS("Create New Zone", getAppImg("es_groups", true)), image: getAppImg("es_groups"))
            if(zApps?.size() && isST()) {
        ...
        if (evtData?.keySet()?.size()) {
            ...
            if (evtData?.echoDevices?.size()) {
                    ...
                    if (!childDevice) {
                        ...
                        if(settings?.autoCreateDevices != false) {
                            try{
                                ...
                                childDevice = addChildDevice("aloneflower52999", childHandlerName, dni, null, [name: childHandlerName, label: devLabel, completedSetup: true])
                            } catch(ex) {
                ...
                if(!isST()) {
                    ...
                    if(!wsDevice) { addChildDevice("aloneflower52999", wsChildHandlerName, "${app?.getId()}|echoSpeaks_websocket", null, [name: wsChildHandlerName, label: "Echo Speaks - WebSocket", completedSetup: true]) }
